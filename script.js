if(document.URL.indexOf("videolezioni") != -1){
		...
		if(arg.sender == "slave"){
			if(typeof arg.content.state !== 'undefined')
				if(arg.content.state.current == "complete"){
					completed++;
					percent.innerHTML = "("+(completed)+"/"+document.getElementsByClassName("h5").length+")";
					innerBar.setAttribute("style", "height:100%; width:"+
							      (((completed)*100)/document.getElementsByClassName("h5").length)+
							      "%; background: #337ab7");
					...
					errList.innerHTML=errList.innerHTML+
						'<li class="h6">'+
							'<a style="color:red;" href = "' + arg.lesson.url + '">'+
								'Lezione ' + arg.lesson.filename.split("/")[1].split(".")[0].split("_")[ 
										arg.lesson.filename.split("/")[1].split(".")[0].split("_").length - 1 ]+
							'</a>'+
						'</li>';
				}
		}
	...
	document.getElementById("ScaricaTutto").addEventListener('click', tmpFunction = function(){ 
		...
		bar.setAttribute("style","display:inline-block; height:20px; width:100%;" +
				 "overflow:hidden; border: 1px solid #e3e3e3;border-radius: 4px;background: #FAFAFA; ");
		button.innerHTML = "Completati";
		percent.innerHTML = "(0/"+document.getElementsByClassName("h5").length+")";
		...
		frame1.addEventListener('load',function(){
			var filename = frame1.contentDocument.getElementById("aflowplayer").getAttribute("href").split("/")[
				frame1.contentDocument.getElementById("aflowplayer").getAttribute("href").split("/").length - 1];
			chrome.runtime.sendMessage({sender: "main", content:{
				...
				if( index < document.getElementsByClassName("h5").length){
				index++;
				frame1.src = document.getElementsByClassName("h5")[index].firstElementChild.getAttribute("href");
				}
		});
		frame1.src = document.getElementsByClassName("h5")[index].firstElementChild.getAttribute("href");
	});
