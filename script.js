if(document.URL.indexOf("videolezioni") != -1){
			...
			if(arg.sender == "slave"){
				if(typeof arg.content.state !== 'undefined')
					if(arg.content.state.current == "complete"){
						...
						innerBar.setAttribute("style", "height:100%; width:"+(((completed)*100)/document.getElementsByClassName("h5").length)+"%; background: #337ab7");
		...
		document.getElementById("ScaricaTutto").addEventListener('click', tmpFunction = function(){ 
			...
			frame1.addEventListener('load',function(){
				var filename = frame1.contentDocument.getElementById("aflowplayer").getAttribute("href").split("/")[document.getElementById("aflowplayer").getAttribute("href").split("/").length - 1];
